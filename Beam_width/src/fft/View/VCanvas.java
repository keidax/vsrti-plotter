...
//        System.out.println(dataPoints.size());
    ...
    public void drawPoint(Graphics2D g, double x, double y) {
        ...
        /*if (dataPoints.containsKey(x)
                && dataPoints.get(x) == y) { */ //TODO not sure what the point of dataPoints is...
            g.setColor(colors[0]);
	...
	public void drawDataSet(int count, Graphics2D g) {
        ...
        for (Double key : keys) {
            ...
            if (rmsPoints.containsKey(key)) {
                //System.out.println("g2cy(y + Sigma*rms) "+g2cy(y+getSigma()*rmsPoints.get(x)));
                drawRms(g, g2cx(key), g2cy(points.get(key) - rmsPoints.get(key) * getSigma() / 2), g2cy(points.get(key) + rmsPoints.get(key) * getSigma() / 2));
            //new SquareOrnament().draw(g, g2cx(key),g2cy(this.points.get(key)));
