...
//	    	(2*height-heihgt+t+b)/2 - b - y*ratio
    ...
    public void drawVerticalMetric(Graphics2D g) {
        ...
        for (int i = 0; i < ((this.getPlotHeight() - 1) / plotStep + 1) / 2; i++) {//horizontal
            ...
            g.drawString("" + Math.round(c2gy((this.getHeight() - bPad - tPad) / 2 + tPad - (i) * plotStep) * 10) / 10.0, 
            x, (int) ((this.getHeight() - bPad - tPad) / 2 + tPad - (i) * plotStep));
        ...
        for (int i = 1; i < ((this.getPlotHeight() - 1) / plotStep + 1) / 2; i++) {//horizontal
            g.drawString("" + Math.round(c2gy((this.getHeight() - bPad - tPad) / 2 + tPad + (i) * plotStep) * 10) / 10.0, 
            x - 5, (int) ((this.getHeight() - bPad - tPad) / 2 + tPad + (i) * plotStep));
        }
        */
    ...
    */
    ...
    public double countHorizontalLabelStep() {// NOT FINISHED
        double count = this.getPlotWidth() / squareWidth;
...
//        return steps[i % steps.length] * (int) Math.pow(10, i / steps.length);
    ...
    */
    ...
    protected void paintPattern(Graphics2D g){
        ...
        for(double i= getMinX();i<=getMaxX();i+=.5){
             g.drawLine(g2cx(i), g2cy(bessel(i)*(points.containsKey(0.0)?points.get(0.0):1)), g2cx(last), g2cy(bessel(last)*(points.containsKey(0.0)?points.get(0.0):1)));
             last=i;
        }
    }
    ...
    protected double sinc(double phi){
        ...
        if(phi==0)
            return 1;
        return Math.pow(Math.sin(Math.PI*view.getD()*Math.sin(phi*Math.PI/180)/adapter.getLambda()), 2)
        	  / Math.pow(         Math.PI*view.getD()*Math.sin(phi*Math.PI/180)/adapter.getLambda(),  2);
    ...
    */
     ...
     * 
     * @param x
     * @return the sum of the first twenty elements of the first-order Bessel function
     */
