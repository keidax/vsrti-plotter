...
public class VisibilityGraph extends Graph {
	...
	/**
	 * @return The number of points in the graph. This is also the number of points used to evaluate the fft.
	 */
    ...
    /**
     * Set the array of Complex Numbers to a new array of complex numbers. This array will be used
     * to determine point values.
    ...
    public void reinicializePoints() {
        ...
        if(getRawPoints().size()!=0)
            for (double i = 0; i <= Math.max(numberOfPoints,getRawPoints().last().getX()/getDeltaBaseline()); i++){
                ...
                this.getPoints2().put(i * getDeltaBaseline() / getLambda(), 0.0);
    ...
    public void fullReset(){
        ...
        rawPoints2 = new TreeSet<Point>();
        rms = new TreeMap<Double, Double>();
        compl = new Complex[this.getPoints().keySet().toArray().length];
        reinicializePoints();
		...
		for(int i = 0; i<num; i++) //This for loop breaks up the String equation. Might make more sense for this to be it's own method.
		{
			if(equation.charAt(i) == '*' || equation.charAt(i) == '/' || equation.charAt(i) == '(' || equation.charAt(i) == ')' || equation.charAt(i) == '+' || equation.charAt(i) == '-' ||  equation.charAt(i) == '^' )
			{
				...
				else
				{
					...
					if(La[index].equals("e")) //changes 'e' to the double version of Euler's constant
						La[index] = Math.E + "";
					else if(La[index].equals("-e")) //changes 'e' to the double version of Euler's constant
						La[index] = "-" + Math.E + "";
    ...
    public void evaluate(String equation, String iEquation)
    {
		...
		{
			if(iEquation.charAt(i) == '*' || iEquation.charAt(i) == '/' || iEquation.charAt(i) == '(' || iEquation.charAt(i) == ')' || iEquation.charAt(i) == '+' || iEquation.charAt(i) == '-' ||  iEquation.charAt(i) == '^' )
			{
				end2 = i;
				if(start2 == end2)
				{					
				...
				else
				{
					...
					if(La2[index2].equals("e")) //changes 'e' to the double version of Euler's constant
						La2[index2] = Math.E + "";
					else if(La2[index2].equals("-e")) //changes 'e' to the double version of Euler's constant
						La2[index2] = "-" + Math.E + "";
...
//        }
    ...
    /**
     * Handles the creation of the Image Graph. It does so by evaluating the fft of the Time Domain's array of complex numbers
     * and setting each point in the image graph (the graphs in the Frequency Domain).
     */
    public void createImageGraph() {
        ...
        for (int i = 0; i < res.length; i++) {
            if(polar)
            {
            	model.imageGraph.getPoints().put(i * this.getLambda() / (getDeltaBaseline() * getPoints().size()), res[i].abs());
            	model.imageGraph.getPoints2().put(i * this.getLambda() / (getDeltaBaseline() * getPoints().size()), res[i].phase());
            }
            else
            {
            	model.imageGraph.getPoints().put(i * this.getLambda() / (getDeltaBaseline() * getPoints().size()), res[i].re());
            	model.imageGraph.getPoints2().put(i * this.getLambda() / (getDeltaBaseline() * getPoints().size()), res[i].im());
            }
        }
    }
    ...
    /**
     * 
     * @param rawPoints - Sorted set of all new raw points.
     */
    ...
    /**
     * Updates all graphs. If polar changes, it will change the graphs from rectangular to polar.
     */
    public void update() {
    	Object[] keys = this.getPoints().keySet().toArray();
    	Complex[] res = FFT.fft(compl);
     ...
     * the real/magnitude graphs in the Time Domain.
     * @param currentPoint
    ...
    /**
     * Changes the x-value of a point. This is called when a point is clicked or dragged on the graph. This handles
     * the imaginary/phase graphs in the Time Domain.
     * @param currentPoint
     * @param toy - new x value.
     */
    ...
    public void importPoints(Double[][] parseFile) {
        ...
        for (int i=0; i <compl.length; i++) {
           ...
           if(polar)
            {
            	this.getPoints().put(parseFile[0][i], compl[i].abs());
            	this.getPoints2().put(parseFile[0][i], compl[i].phase());
            }
            else
            {
            	this.getPoints().put(parseFile[0][i], compl[i].re());
            	this.getPoints2().put(parseFile[0][i], compl[i].im());
            }
        }
...
//        this.setDeltaBaseline(this.countDeltaBaseline());
    ...
    public String toString() {
        ...
        for (Double key : keys) { // X Y RMS
            s += key * this.lambda + " " + compl[i].re() + " " + compl[i].im() + "\n";
            i++;
        }
        return s;
    }
    ...
    /**
     * 
     * @return Treemap<Double,Double> of data points from the real/magnitude graph.
     */
    ...
    /**
     * 
     * @return Treemap<Double,Double> of data points from the imaginary/phase graph.
     */
