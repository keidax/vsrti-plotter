...
public class Adapter {
    ...
    {
    	this.model.visibilityGraph.setPolar(radio);
    }
    ...
    public TreeMap<Double, Double> getVisibilityGraphDataPoints2() {
        return this.getModel().getVisibilityGraph().getDataPoints2();
    }
    ...
    public TreeMap<Double, Double> getImageGraphPoints2() {
        TreeMap<Double, Double> reduced = new TreeMap<Double, Double>();
        Set<Double> keys = getModel().getImageGraph().getPoints2().keySet();
        int sum = keys.size();
        int count = 0;
        for (Double key : keys) {
            count++;
            reduced.put(key, getModel().getImageGraph().getPoints2().get(key));
            if (count == sum / 2) {
                break;
            }
        }
        reduced.putAll(getModel().getImageGraph().getPoints2().headMap(getModel().getImageGraph().getPoints2().lastKey() / 2 + 1));
        //System.out.println("Size of reduced "+reduced.size());
        //return reduced;
    ...
    public void moveVisibilityPoint2(double currentPoint, double toy) {
        this.getModel().getVisibilityGraph().movePoint2(currentPoint, toy);
    }
    ...
    public void moveImagePoint2(Double currentPoint, double toy) {
        this.getModel().getImageGraph().movePoint2(currentPoint, toy);
    }
    ...
    public void fullReset() {
        ...
        this.getModel().getVisibilityGraph().fullReset();
