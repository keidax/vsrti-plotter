...
import fft.View.Canvas;
import fft.View.SquareOrnament;
import fft.View.VCanvas;
...
//	    	(2*height-heihgt+t+b)/2 - b - y*ratio
    ...
    public void drawXAxis(Graphics2D g) {
        ...
        for (int i = 0; i < (this.getPlotWidth() - 1) / steps + 1; i++) {//horizontal
            ...
            g.setColor(Color.BLACK);
            g.drawString("" + df.format(Math.round((i) * lstep * 100) / 100.0), (int) (this.getLeftShift() + (i) * steps - xLabelWidth / 4), this.getHeight() - 17);
        }
