...
public class VisibilityGraph extends Graph {
    ...
    public TreeMap<Double, Double> rms = new TreeMap<Double, Double>(),
            griddedRms = new TreeMap<Double, Double>();
    ...
    public void createImageGraph() {
        ...
        for (int i = 0; i < keys.length; i++) {
            // if(gridedRms==null)
            if (getGriddedRms() == null
                    || !getGriddedRms().containsKey(keys[i])) {
                data[0][i] = data[1][i] = data[2][i] = getPoints().get(keys[i]);
            } else {
                data[0][i] =
                        getPoints().get(keys[i]) + getGriddedRms().get(keys[i])
                                / 2;
        ...
        for (int i = 0; i < res[0].length; i++) {
            ...
            model.imageGraph.getPoints().put(
                    i * getLambda()
                            / (getDeltaBaseline() * getPoints().size() * 2),
            ...
            s +=
                    key
                            * lambda
                            ...
                            + getPoints().get(key)
                            + " "
                            + (getGriddedRms().containsKey(key)
                                    ? getGriddedRms().get(key) : 0.0) + "\n";
        }
        s += "\n\nANGLE_INTENSITY";
