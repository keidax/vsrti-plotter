			...
			public void actionPerformed(ActionEvent e) {
                ...
                if (fileChooser.showSaveDialog(canvas) == JFileChooser.APPROVE_OPTION) {
                    ...
                    try {
                        ...
                        EpsGraphics2D g = new EpsGraphics2D("Title", f, 0,0, (int)(getWidth()*scale/2),(int)(getHeight()*scale/2));
                        scale = 4;
                        rPad =(int)(rPad*3);
                        g.scale(0.24, 0.24);
...
//    	//draw axes
//        g2.setColor(Color.BLACK); 
//        g2.drawLine(this.getLeftShift(), this.getHeight()-Canvas.bPad, this.getLeftShift()+this.getPlotWidth(), this.getHeight()-Canvas.bPad);//horizontal// draw horizontal axis
//        
//        i=0;
    ...
    */
    ...
    protected void paintVis(Graphics2D g){
        ...
        for(double i= getMinX();i<=getMaxX();i+=.1){
            //g.drawLine(g2cx(i), g2cy(visModel(i)*(points.containsKey(0.0)?points.get(0.0):1)), 
        	//g2cx(last), g2cy(visModel(last)*(points.containsKey(0.0)?points.get(0.0):1)));
