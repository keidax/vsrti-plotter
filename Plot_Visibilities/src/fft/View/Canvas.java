...
public abstract class Canvas extends JPanel implements MouseListener,
    ...
    public void drawDataSet(Graphics2D g) {
        ...
        for (Double key : keys) {
            ...
            // g2cx(key),g2cy(this.getPoints().get(key)));
    ...
    public void drawXAxis(Graphics2D g) {
        ...
        for (int i = 0; i < Math.round(getPlotWidth() / squareWidth + 0.5); i++) {// horizontal
            ...
            g.drawString(
                    lString,
                    (int) (getLeftShift() + i * steps - fm.stringWidth(lString) / 2),
                    getHeight() - bPad + fm.getAscent() + fm.getLeading() + 5);
        }
        // draw axis title
        g.drawString(" " + xAxisTitle + " ", getLeftShift() + getPlotWidth()
                / 2 - 50, getHeight() - 10);
    ...
    public void drawVerticalMetric(Graphics2D g) {
        ...
        for (int i = 0; i <= getPlotHeight() / squareWidth; i++) {// horizontal
            ...
            // (i) * plotStep) * 10) / 10.0;
            String tempString =
                    df.format(c2gy(getHeight() - bPad - i * squareWidth));
            g.drawString(tempString, lPad - fm.stringWidth(tempString) - 5,
                    getHeight() - bPad - i * squareWidth + fm.getAscent() / 2);
        }
    }
    ...
    public void drawVerticalLabel(Graphics2D g) {
        ...
        int translateDown =
                (getPlotHeight() + g.getFontMetrics().stringWidth(yAxisTitle)) / 2;
     ...
     * (getPoints() == null || getPoints().size()==0) { if(view.showVis){ max =
    ...
    /*
     * public Double getMinYPoint() { if (getPoints().size() == 0) { return
     * (double) -Canvas.defaultY; } double min =
     * this.getPoints().firstEntry().getValue(); Set<Double> keys =
     * this.getPoints().keySet(); for (Double key : keys) { if
     * (this.getPoints().get(key) < min) { min = this.getPoints().get(key); } }
     * return min; }
     */
    ...
    public void mouseDragged(MouseEvent evt) {
        ...
        if (getCurrentPoint() != null) {
            // double tox = Math.min(Math.max(getLeftShift(), mCanx),
            // getLeftShift() + getPlotWidth());
            double toy = Math.min(Math.max(tPad, mCany), getHeight() - bPad);
            adapter.moveVisibilityPoint(getCurrentPoint(), c2gy(toy));
            // getCurrentPoint().movePoint(getCurrentPoint(),
            // c2gx(tox),c2gy(toy,getCurrentPoint().getDataSet()));
    ...
    protected void doPaint(Graphics g) {
        ...
        g2.drawString(graphTitle, (getWidth() - g2.getFontMetrics()
                .stringWidth(graphTitle)) / 2, (tPad + g2.getFontMetrics()
                .getHeight() / 2) / 2);
        g2.setColor(colors[0]);
        drawDataSet(g2);
        if (view.showVis) {
    ...
    protected void paintVis(Graphics2D g) {
        ...
        for (double i = getMinX(); i <= getMaxX(); i += .1) {
            ...
            g.drawLine(g2cx(i), g2cy(visModel(i)), g2cx(last),
                    g2cy(visModel(last)));
            last = i;
        }
    }
    ...
    protected double visModelOld(double blam) {
        ...
        double tA =
                view.T1 * Math.sqrt(2 / Math.pow(X1, 2) * (1 - Math.cos(X1)));
        double tB =
                view.T2 * Math.sqrt(2 / Math.pow(X2, 2) * (1 - Math.cos(X2)));
