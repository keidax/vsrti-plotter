...
public abstract class Canvas1 extends JPanel implements MouseListener, MouseMotionListener {
    ...
    public Canvas1(Viewer v, Adapter a, TreeMap<Double, Double> g) {
        canvas = this;
        setPoints(g);
        setAdapter(a);
        this.setView(v);
...
//	    	(2*height-heihgt+t+b)/2 - b - y*ratio
    ...
    public void drawXAxis(Graphics2D g) {
        ...
        //g.drawLine(yLabelWidth+(yLabelWidth/2), (this.getHeight()-bPad)/2, this.getWidth()-yLabelWidth-rPad, (this.getHeight()-bPad)/2);
        ...
        for (int i = 0; i < (this.getPlotWidth() - 1) / steps + 1; i++) {//horizontal
            ...
            g.setColor(Color.BLACK);
            g.drawString("" + Math.round((i - (this.getPlotWidth()) / steps/2) * (lstep) * 100) / 100.0, (int) (this.getLeftShift() + (i) * steps - xLabelWidth / 4), this.getHeight() - 17);
        }
    ...
    public void drawYAxis(int count, Graphics2D g) {
        ...
        //new SquareOrnament().draw(g, lPad+(((int)(count/2))*yLabelWidth)+23, 10);
    }

    public void drawVerticalLine(int x, double steps, Graphics2D g) {
        g.drawLine(x, Canvas1.tPad, x, this.getHeight() - Canvas1.bPad);//vertical
        g.setColor(Color.LIGHT_GRAY);
        for (int i = 0; i < (this.getPlotHeight() - 1) / steps + 1; i++) {//horizontal
            g.drawLine(x - 2, (int) (this.getHeight() - Canvas1.bPad - (i) * steps), x + 2, (int) (this.getHeight() - Canvas1.bPad - (i) * steps));
        }
    }
    ...
    public void drawVerticalMetric(int x, double strStep, double plotStep, Graphics2D g) {
        ...
        for (int i = 0; i < ((this.getPlotHeight() - 1) / plotStep + 1) / 2; i++) {//horizontal
            //g.drawString(""+Math.round(i*strStep*10)/10.0,x, (int)((this.getHeight()-FFTCanvas.bPad-tPad)/2+tPad-(i)*plotStep));
            g.drawString("" + Math.round(c2gy((this.getHeight() - Canvas1.bPad - tPad) / 2 + tPad - (i) * plotStep) * 10) / 10.0, x, (int) ((this.getHeight() - Canvas1.bPad - tPad) / 2 + tPad - (i) * plotStep));
            //g.drawString(""+Math.round(i*strStep*10)/10.0,x, g2cy((i*plotStep)));
            //g.drawString(""+Math.round((i*strStep)*10)/10.0,x, (int)(g2cy(i)*plotStep));
        }
        for (int i = 1; i < ((this.getPlotHeight() - 1) / plotStep + 1) / 2; i++) {//horizontal
            g.drawString("" + Math.round(c2gy((this.getHeight() - Canvas1.bPad - tPad) / 2 + tPad + (i) * plotStep) * 10) / 10.0, x - 5, (int) ((this.getHeight() - Canvas1.bPad - tPad) / 2 + tPad + (i) * plotStep));
        }
        //g.drawString(""+Math.round(-i*strStep*10)/10.0,x, (int)((this.getHeight()-FFTCanvas.bPad-tPad)/2+tPad-(-i)*plotStep));}
    }
    ...
    protected int getPlotHeight() {
        return this.getHeight() - Canvas1.tPad - Canvas1.bPad;
    }
    ...
    public void mouseMoved(MouseEvent evt) {
        ...
        if (getPointOnGraph(mCanx,mCany) != null) {
            ...
            this.setToolTipText("["+p+";"+this.getPoints().get(p)+"]");
        }
        if (getVerticallyPointOnGraph(mCanx, mCany) != null) {
