...
public abstract class Canvas extends JPanel implements MouseListener, MouseMotionListener {
 ...
 item2.addActionListener(new ActionListener() {
    ...
    public void drawXAxis(Graphics2D g) {
        ...
        double min = 0;
        if(this.xAxis.equals("Frequency"))
        	min = Viewer.viewer.currentNode.fStart;
        for (int i = 0; i < (this.getPlotWidth() - 1) / steps + 1; i++) {
            g.setColor(Color.LIGHT_GRAY);
            g.drawLine((int) (this.getLeftShift() + (i) * steps), tPad, (int) (this.getLeftShift() + (i) * steps), this.getHeight() - bPad);
            g.setColor(Color.BLACK);
            if(this.xAxis.equals("Frequency (MHz)") && i == 0)
            	g.drawString(Viewer.viewer.currentNode.fStart + "+", 0, this.getHeight()-17);
            if(this.graphTitle.equals("Beam Width: Average Antenna Temperature vs. Angle"))
            {
            	 g.drawString("" + df.format((i * lstep * 100 / 100.0) + Viewer.viewer.min), (int) (this.getLeftShift() + (i) * steps - xLabelWidth / 4), this.getHeight() - 17);
            }
            else
            {
            	 g.drawString("" + df.format((i * lstep * 100 / 100.0)), (int) (this.getLeftShift() + (i) * steps - xLabelWidth / 4), this.getHeight() - 17);
            }
    ...
    public void drawVerticalLine(int x, double steps, Graphics2D g) {
        g.drawLine(x, Canvas.tPad, x, this.getHeight() - Canvas.bPad);
        g.setColor(Color.LIGHT_GRAY);
        for (int i = 0; i < (this.getPlotHeight() - 1) / steps + 1; i++) {
    ...
    public void drawVerticalMetric(int x, double strStep, double plotStep, Graphics2D g) {
        g.setFont(new Font(g.getFont().getFontName(), 0, 11));
        for (int i = 0; i < ((this.getPlotHeight() - 1) / plotStep + 1) / 2; i++) {
    ...
    public double getMaxX() {
        ...
        maxX = this.getPoints().lastKey();
        if(this.xAxis.equals("Frequency (MHz)"))
        	return this.getPoints().lastKey();
        else
        	return Math.ceil(maxX/(this.getPlotWidth() / squareWidth + 1))*(this.getPlotWidth() / squareWidth + 1);
      //  return this.getPoints().lastKey();
    }
	...
	public void mousePressed(MouseEvent evt) {
        ...
        double p = 0;
        	...
        	if(Viewer.viewer.choosingDelete && getPointOnGraph(mCanx, mCany) != null && evt.getButton() != MouseEvent.BUTTON2)
        	{
        		 p = getPointOnGraph(mCanx, mCany)
        		 ;
        		if(Viewer.viewer.deleting[((int)p) -1] != -1)
        		{
        			drawPoint((Graphics2D)getGraphics(),p, this.getPoints().get(p),false);
        			Viewer.viewer.deleting[((int)p) -1] = -1;
        			
        		}
        		else if(Viewer.viewer.deleting[((int)p) -1] == -1 )
        		{
        			drawPoint((Graphics2D)getGraphics(),p, this.getPoints().get(p));
        			Viewer.viewer.deleting[((int)p) -1] = 0;
    ...
    public void mouseDragged(MouseEvent evt) {
        ...
        mCany = evt.getY();
        double p = 0;
            if (evt.getButton() != MouseEvent.BUTTON2 && getVerticallyPointOnGraph(mCanx, mCany) != null && Viewer.viewer.choosingDelete){
    ...
    public void mouseMoved(MouseEvent evt) {
        ...
        mCany = evt.getY();
        double min = 0;
        if(this.xAxis.equals("Frequency") && Viewer.viewer.currentNode != null)
        	{
        		min = Viewer.viewer.currentNode.fStart;
        		df = new DecimalFormat("#.###");
        	}
        if( Viewer.viewer.vGraph.graphTitle.equals("Averge TA vs. Order") && getPointOnGraph(mCanx,mCany) != null)
        {
        	double p = getVerticallyPointOnGraph(mCanx, mCany);
            this.setToolTipText("["+df.format(p+min)+"; "+df.format(this.getPoints().get(p))+"]  Data Block: " + Viewer.viewer.taPlotted[(int)p] + ")") ;
        }
        else if(this.graphTitle.equals("Beam Width: Average Antenna Temperature vs. Angle") && getPointOnGraph(mCanx,mCany) != null)
        {
        	double p = getVerticallyPointOnGraph(mCanx, mCany);
            this.setToolTipText("["+df.format(p+Viewer.viewer.min)+"; "+df.format(this.getPoints().get(p))+"]");
        }
        else if (getPointOnGraph(mCanx,mCany) != null) {
            double p = getVerticallyPointOnGraph(mCanx, mCany);
            this.setToolTipText("["+df.format(p+min)+"; "+df.format(this.getPoints().get(p))+"]");
        }
...
////// D
    ...
    protected void paintSinc(Graphics2D g){
        ...
        for(double i= -maxX+getPlotWidth()/80;i<maxX;i+=.5){
             g.drawLine(g2cx(i), g2cy(sinc(i)*(points.containsKey(0.0)?points.get(0.0):1)), g2cx(last), g2cy(sinc(last)*(points.containsKey(0.0)?points.get(0.0):1)));
             last=i;
        }
    }
    /**
    ...
    protected double sinc(double phi){
    	if(phi==0)
            return 1;
    	  return 100*Math.pow(Math.sin(Math.PI*view.getD()*Math.sin(phi*Math.PI/180)/adapter.getLambda()), 2)/Math.pow(Math.PI*view.getD()*Math.sin(phi*Math.PI/180)/adapter.getLambda(),2);
