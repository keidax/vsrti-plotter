...
public class FileTable extends JTable implements TableModelListener{
   ...
   @Override
	   ...
	   if(me.getEditingColumn() == 2)
		   {
		   			try
		   			{
		   				...
		   				(((TableModel)me.getModel()).list.getNode(this.getEditingRow())).angle = Double.parseDouble((String) this.getCellEditor().getCellEditorValue());
		   			}
		   			catch(Exception ex)
		   			{
    ...
    /**
     * Updates the Visibility graph with the points of the selected data set. This is necessary if any points are
     * deleted or changed.
     */
    ...
    /**
     * Plots the spectrum of the selected data block. That is Temperature vs. Frequency.
     */
    ...
    public boolean plotBeam()
    {
		...
		double min = ((TableModel)me.getModel()).viewer.min;System.out.println(min);
		for(int i = 0; i< index.length; i++)
		{
				avg = ((ListNode)((TableModel)me.getModel()).getValueAt(index[i])).getAverageOverFrequency();
				angle = ((ListNode)((TableModel)me.getModel()).getValueAt(index[i])).angle - min;
    ...
    /**
     * Deletes the selected data blocks as well as the AVG and SUB data blocks that were created using one
     * or more of the selected data blocks.
     */
    public void delete()
    {
    	...
    	for(int i = 0; i < selected.length; i++)
    		{
    			mess = mess + selected[i] + ") " + ((TableModel)me.getModel()).viewer.list.getNode(selected[i]).title + "\n";
    	...
    	int delete = JOptionPane.showConfirmDialog(null, "Are you sure you want to delete these data blocks? The associated SUB and AVG data blocks will also be deleted.\n" + mess);
    		...
    		while(node != null)
    		{
    			if(node.title.contains("AVG") || node.title.contains("SUB"))
    			{
    				...
    				for(int i = 0; i < temp.length; i++)
    				{
    					if(numb.contains(" " + temp[i] + " "))
    					{
    						((TableModel)me.getModel()).viewer.list.remove(node.title);
    						break;
    					}
