...
public class Viewer extends JFrame implements ModelListener, ActionListener {
    ...
    public JButton bSave, bOpen, bExit, bReset, bDelete, bSelectEnd, bInstruction, bAbout, bAvgFreq, bAvgBlocks, bSpectrum, bTime, bSubtract, bUndo, bBeam;
    ...
    public Viewer(Adapter a, String title) {
  		                ...
  		                select.addActionListener(new ActionListener() {
  		                    ...
  		                    public void actionPerformed(ActionEvent arg0) {
  		                    	...
  		                    	else
  		                    	{
  		                    		...
  		                    		{
  		                    			JOptionPane.showMessageDialog(null, "Both data blocks must have the same data length, start frequency, and step frequency");
  		                    			return;
  		                    		}
  		                    		else
  		                    		{
        ...
        bBeam.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
		                ...
		                JButton enter = new JButton("Cancel");
		                JButton select = new JButton("Select");
		                TableModel t = new TableModel(Viewer.viewer, list, 3);
            			...
            			int opt = JOptionPane.showOptionDialog(null, "Would you like to plot vs. azimuth or vs. elevation?", "Plot Beam", JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null, o, o[2]);
            			String[] temp;
            			int count = 0;
            			if(opt == 0)
            			{
...
//  		                    		final JFrame jFr = new JFrame("Beam Model Parameters");
...
//  		      		                fLambda = new JTextField(viewer.adapter.getLambda() + "");
...
//  		    		                row6.setLayout(new BoxLayout(row6, BoxLayout.Y_AXIS));
//  		    		                row5.setLayout(new BoxLayout(row5, BoxLayout.Y_AXIS));
//  		    		                row1.setLayout(new BoxLayout(row1, BoxLayout.X_AXIS));
...
//  		    		                row1.add(row6);
//  		    		                row6.add(jl);
//  		    		                row6.add(Box.createRigidArea(new Dimension(20, 5)));
//  		    		                row6.add(jl2);
//  		    		                row1.add(Box.createRigidArea(new Dimension(20, 5)));
//  		    		                row1.add(row2);
//  		    		                row1.add(row3);
      ...
      bSave.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent arg0) {
                    	...
                    	if(jTable.getSelectedRows().length !=0)
                    		opt = JOptionPane.showOptionDialog(null, "Save only the selected data block? (Choose no to save all)", "SRT Plotter - Save File", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE, null, null, null);
                    	if(opt == 0)//yes
                    	{
                    		Viewer.viewer.lDelta.setText("");
      ...
      bAvgBlocks.addActionListener(new ActionListener() {
		                       ...
		                       if(selected.length > 0)
		                       {
		                    	    ListNode[] nodes = new ListNode[selected.length];
			                       	...
			                       	for(int i = 0; i< data.length; i++)
			                       	{
			                       		...
			                       		{
			                       			if(nodes[j-1].data.length != nodes[j].data.length || nodes[j-1].fStart != nodes[j].fStart || nodes[j-1].fStep != nodes[j].fStep)
			                       				{
			                       					JOptionPane.showMessageDialog(null, "All blocks of data must have the same length, start frequency, and step for Average Blocks of Data.");
			                       					return;
			                       				}
			                       	...
			                       	Viewer.viewer.jTable.addNotify();
  			                        jTable.clearSelection();
            ...
            @Override
            	...
            	DecimalFormat df = new DecimalFormat("#.####");
            	   if(currentNode == null)
            		   ...
            		   Viewer.viewer.lDelta.setText("Average Over Frequency: "  + df.format(currentNode.getAverageOverFrequency()));
                }
        });
        ...
        bReset.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
            	...
            	if(Viewer.viewer.vGraph.graphTitle.equals("Spectrum: Antenna Temperature vs. Frequency") || Viewer.viewer.vGraph.graphTitle.equals("Averge TA vs. Order"))
            		return;
                choosingDelete = false;
                ListNode node;
        ...
        bUndo.addActionListener(new ActionListener() {
            ...
            @Override
            public void actionPerformed(ActionEvent arg0) {
            	if(currentNode == null || Viewer.viewer.vGraph.graphTitle.equals("Spectrum: Antenna Temperature vs. Frequency") || Viewer.viewer.vGraph.graphTitle.equals("Averge TA vs. Order"))
            		return;
		                ...
		                select.addActionListener(new ActionListener() {
		                    ...
		                    public void actionPerformed(ActionEvent arg0) {
		                       ...
		                       if(selectedRows.length > 0)
		                       {
		                    	   ...
		                    	   {
		                    		   if(list.getNode(selectedRows[i-1]).data.length != list.getNode(selectedRows[i]).data.length || list.getNode(selectedRows[i-1]).fStart != list.getNode(selectedRows[i]).fStart || list.getNode(selectedRows[i-1]).fStep != list.getNode(selectedRows[i]).fStep)
		                    				   {
		                    			   			JOptionPane.showMessageDialog(null, "All blocks of data must have the same length, start frequency, and step for Average Blocks of Data.");
		                    			   			return;
		                    				   }
			                	   ...
			                	   jf.setVisible(false);
			                	   canDelete = true;
			                	   JOptionPane.showMessageDialog(null, "Select the end channels to be deleted and then click the 'delete selected channels' button.");
		                       }
		                       else
		                       {
		                    	   JOptionPane.showMessageDialog(null, "You must select one or more data blocks.");
