...
public abstract class Canvas extends JPanel implements MouseListener, MouseMotionListener {
 ...
 item2.addActionListener(new ActionListener() {
    ...
    public void drawXAxis(Graphics2D g) {
        ...
        for (int i = 0; i < (this.getPlotWidth() - 1) / steps + 1; i++) {
            ...
            if(this.xAxis.equals("Frequency (MHz)") && i == 0)
            	g.drawString(View.viewer.currentNode.fStart + "+", 0, this.getHeight()-17);
            if(this.graphTitle.equals("Beam Width: Average Antenna Temperature vs. Angle"))
            {
            	 g.drawString("" + df.format((i * lstep * 100 / 100.0) + View.viewer.min), (int) (this.getLeftShift() + (i) * steps - xLabelWidth / 4), this.getHeight() - 17);
            }
            else
            {
    ...
    public void mouseMoved(MouseEvent evt) {
        ...
        mCany = evt.getY();
        double min = 0;
        if(this.xAxis.equals("Frequency") && View.viewer.currentNode != null)
        	{
        		min = View.viewer.currentNode.fStart;
        ...
        if( View.viewer.vGraph.graphTitle.equals("Averge TA vs. Order") && getPointOnGraph(mCanx,mCany) != null)
        {
            ...
            this.setToolTipText("["+df.format(p+min)+"; "+df.format(this.getPoints().get(p))+"]  Data Block: " + View.viewer.taPlotted[(int)p] + ")") ;
        }
        else if(this.graphTitle.equals("Beam Width: Average Antenna Temperature vs. Angle") && getPointOnGraph(mCanx,mCany) != null)
        {
        	double p = getVerticallyPointOnGraph(mCanx, mCany);
            this.setToolTipText("["+df.format(p+View.viewer.min)+"; "+df.format(this.getPoints().get(p))+"]");
        }
